...
public class GameLogic : MonoBehaviour
{
    ...
    void InitNodeList()
    {
        ...
        {
            Offset.value = GameSettingModule.Offset;
            input.text = GameSettingModule.Offset.ToString();
            Debug.Log(GameSettingModule.Offset);
        }
        //给每一个节点做偏移
        Debug.Log("玩家自定义偏移值：" + GameSettingModule.Offset);
        for (int i = 0; i < NodeList.Count; i++)
        {
            NodeList[i].StartTime += GameSettingModule.Offset;
            NodeList[i].EndTime += GameSettingModule.Offset;
        }
    ...
    private void CheckForFinish()
    {
        if (TimeFromStart > Music.length+2)
        {
            if (SceneManager.GetActiveScene().name == "Practice")
            {
                SceneModule.Instance.LoadScene("Lobby");
            }
            else
            {
    ...
    public void ChangeInputFeildOffset()
    {
    ...
    {
        GameSettingModule.Offset = float.Parse(input.text);
